...
class TaskOut(BaseModel):
    ...
    task_id:         int
    task_name:       str
    task_type:       str = "install"    # install | uninstall | run_command | registry | cleanup
    ...
    command:         Optional[str] = None  # 脚本内容（bat/cmd/powershell）
    interpreter:     Optional[str] = None  # bat | cmd | powershell
    registry_ops:    Optional[str] = None  # JSON 文本：[{action,root,subkey,name,value,type}]
    cleanup_paths:    Optional[str] = None  # JSON 文本：[{path,recursive}]
    run_as:          Optional[str] = "system"  # system | user


# ── 命令类任务：注册表操作 / 清理路径（仅用于创建任务时的入参校验）──
class RegistryOp(BaseModel):
    action: str                          # set | delete
    root:   str = "HKLM"                 # HKLM | HKCU
    subkey: str
    name:   Optional[str] = None        # 值名；None 表示"(默认)"
    value:  Optional[str] = None        # set 时的值（delete 时忽略）
    type:   Optional[str] = "string"    # string | dword | qword | binary | expand


class CleanupPath(BaseModel):
    path:      str
...
class TaskCreate(BaseModel):
    name:          str
    task_type:     str                       # install | uninstall | run_command | registry | cleanup
    target_type:   str = "client"            # client | group | all
    target_ids:    list[int] = []            # target_type=client/group 时的目标 id 列表
    # 安装/卸载
    package_id:    Optional[int] = None
    uninstall_target: Optional[str] = None
    # 命令类
    command:       Optional[str] = None
    interpreter:   Optional[str] = None
    registry_ops:  list[RegistryOp] = []
    cleanup_paths: list[CleanupPath] = []
    run_as:        str = "system"            # system | user
    # 通用
...
# ── 客户端自更新信息（GET /api/client/update）──
class ClientUpdateOut(BaseModel):
    available: bool = False
    version:   Optional[str] = None
    url:       Optional[str] = None
    hash:      Optional[str] = None          # SHA256(更新包)，hex 小写
...
class TaskResultRequest(BaseModel):
    ...
    deferred:    bool = False
    # ── 状态机补全：Agent 上报自身版本号 + 后校验快照 ──
    executor_version: Optional[str] = None   # Agent 当前版本号（ClientVersion.Current）
    verify_snapshot:  Optional[dict] = None  # registry_fix: 写入后读回的实际值 {"root":"HKLM","subkey":"...","name":"...","value":"..."}
...
class AISettingsOut(BaseModel):
    """AI 设置读取（token 永不明文返回，只给末4位+是否已配置）"""
    openclaw_url: str
    openclaw_model: str
...
class AISettingsIn(BaseModel):
    """AI 设置保存。openclaw_token 留空/不传 = 保持原值不变；显式传空字符串也视为不变（用 clear_token 单独清除）"""
    openclaw_url: str
    openclaw_model: str
